...
def getReport(username, idNo):
    ...
    if idNo == 'q':
        ...
        checkEncryption(idNo)
        return
        #downloadFile(username, idNo)

def checkEncryption(idNo):
    s=requests.Session()
    ...
    if rpost.text == "False":
        ...
        downloadEncryptedFile(idNo)
        pass

def downloadFile(idNo):
    s=requests.Session()
    ...
    with open('downloadFile', 'wb') as handle:
        ...
        for block in response.iter_content(1024):
            handle.write(block)
    print("The file was downloaded to filename downloadFile")

def downloadEncryptedFile(idNo):
    print("here")
    s=requests.Session()
    response = s.post('http://127.0.0.1:8000/download_files_fda', data={'id': idNo}, stream=True)
    ...
    for block in response.iter_content(1024):
        text = handle.read(block) #write?
    # decrypt the text
    decrypted = privateKey.decrypt(text)
    # write to a file
    with open('downloadFile', 'wb') as wf:
        wf.write(decrypted)
